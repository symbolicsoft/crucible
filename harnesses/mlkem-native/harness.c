/*
 * Crucible ML-KEM Harness for mlkem-native (PQ Code Package)
 *
 * This harness wraps mlkem-native's single compilation unit build.
 * We build for ML-KEM-768 by default using the derand API.
 *
 * mlkem-native exposes:
 *   crypto_kem_keypair_derand(pk, sk, coins)  -- 64 bytes coins
 *   crypto_kem_enc_derand(ct, ss, pk, coins)  -- 32 bytes coins
 *   crypto_kem_dec(ss, ct, sk)
 *
 * With namespace: PQCP_MLKEM_NATIVE_MLKEM768_keypair_derand, etc.
 * But via the SUPERCOP API: crypto_kem_keypair_derand, etc.
 */
...
/* mlkem-native public API header */
...
#define PK_BYTES  MLKEM768_PUBLICKEYBYTES   /* 1184 */
#define SK_BYTES  MLKEM768_SECRETKEYBYTES   /* 2400 */
#define CT_BYTES  MLKEM768_CIPHERTEXTBYTES  /* 1088 */
#define SS_BYTES  MLKEM_BYTES               /* 32 */
...
static void handle_keygen(cJSON *inputs, cJSON *params) {
    ...
    if (rand_hex) {
        ...
        rc = crypto_kem_keypair_derand(pk, sk, coins);
    } else {
...
static void handle_encaps(cJSON *inputs, cJSON *params) {
    ...
    if (rand_hex) {
        ...
        rc = crypto_kem_enc_derand(ct, ss, pk, coins);
    } else {
...
static void handle_decaps(cJSON *inputs, cJSON *params) {
    ...
    uint8_t ss[SS_BYTES];
    int rc = crypto_kem_dec(ss, ct, sk);
    if (rc != 0) {
...
int main(void) {
    ...
    cJSON_AddStringToObject(hs, "implementation", "mlkem-native-pqcp");
